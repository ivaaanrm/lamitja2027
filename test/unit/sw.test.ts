...
describe('service worker · app shells', () => {
  ...
  it('treats a ClientRouter tab switch as the document it is', async () => {
    // A tab tap is not a navigation here: `ClientRouter` asks with a plain `fetch()`, so
    // there is no navigate mode, no document destination and no `Accept: text/html`.
    // Matched on those alone it fell through to the network, and offline the router gave
    // up and reloaded the whole document instead of swapping the body.
    sw.fetch.mockResolvedValueOnce(html('<p>progreso</p>'))
    const swap = await sw.request('/progreso')
